...
DATABASE_ID2 = os.getenv('DATABASE_ID2') #for testing modules database (databaseid 2)
...
sorted_pages = sorted(pages, key=lambda x: (
    x["properties"]["Launch Date"]["date"] is None, 
    x["properties"]["Launch Date"]["date"]["start"] if x["properties"]["Launch Date"]["date"] is not None else ''
))


def module_extract(sorted_pages):
    modules_info = []
    for page in sorted_pages:
        ...
        metadata = (page_id, page_type, status, name, grades, launch_date)
        modules_info.append(metadata)
        #print(f"Type: {page_type}, Status: {status}, Name: {name}, Grades: {', '.join(grades)}, Launch Date: {launch_date}")
    return modules_info

module_data = module_extract(sorted_pages)
#print(module_data)
